...
这个 Hermes model-provider plugin 会把 Hermes 请求路由到 `hermes_proxy.py`，并在不修改 Hermes 源码的情况下添加轨迹 headers。
...
插件依赖 Hermes 官方的 `ProviderProfile.build_api_kwargs_extras()` 扩展点。这个扩展点可以返回 OpenAI client kwargs，例如 `extra_headers`。
