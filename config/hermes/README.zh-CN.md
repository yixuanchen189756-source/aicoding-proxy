...
编辑 Hermes 的配置文件，通常是 `~/.hermes/config.yaml`，选择这个 provider：
...
`auxiliary.title_generation` 会把 Hermes 的后台标题生成请求固定到同一个代理地址。它不是轨迹 header 的必要条件，但可以避免 Hermes 生成标题时回退到其他 provider。

## Tailnet 和代理绕过

如果 proxy host 是 tailnet 或局域网地址，启动 Hermes 前设置 `NO_PROXY`，避免 Python/httpx 把请求发到系统 HTTP 代理：
...
## 需要改 `proxy/config.yaml` 吗？

通常不需要。

只有当你想改变代理监听端口、上游 backend、usage 文件、认证 key 或轨迹目录时，才需要改 `proxy/config.yaml`：
...
用该 provider 启动 Hermes 后发送一条消息。代理应该写出：
