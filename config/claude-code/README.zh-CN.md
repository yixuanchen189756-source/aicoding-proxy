...
本目录包含 Claude Code hook。代理用它把 Claude Code 的 `session_id` 和模型请求绑定起来。

Claude Code 和 OpenCode、Hermes 不同：

- 模型请求使用 Anthropic Messages API
- 当前 session ID 不会自然出现在 HTTP header 中
- session 生命周期信息可以通过 Claude Code hooks 获取

因此集成使用两条通道：

1. **每个模型请求上的 HTTP headers** 标识当前 run/workspace。
2. **Session hooks** 把 Claude Code `session_id` 事件上报给代理。
...
config/claude-code/
  hooks/
    claude_code_session_hook.bat
      Windows wrapper。创建日志目录、记录基础诊断信息，并调用 Python hook。
...
Claude Code 的模型请求应该发送到 Claude Code profile：
...
hook 脚本默认使用 `http://127.0.0.1:8906/_agent/session-event`。如果代理运行在其他主机上，请设置 `CLAUDE_CODE_SESSION_EVENT_URL`，不要直接改脚本。
...
逗号分隔可能会被 Claude Code 当成一个畸形的 `x-agent-name` header，而不是多个独立 header。

## 环境变量

launcher、shell profile、`.env` loader 或终端脚本必须设置：
...
hook 使用环境变量里的 `CLAUDE_CODE_RUN_ID`，以及 Claude Code hook payload 里的 `session_id`。
...
每次 Claude Code 发出 session hook 事件时，hook 都会把最新 session ID 上报给代理。代理会更新当前 run ID 对应的 active mapping。

这意味着代理不需要用单独的 `run_id` 目录保存真实对话轨迹。`run_id` 只是绑定 key：
...
Windows 上请配置 Claude Code 通过 PowerShell 运行 `.bat` hook。否则某些安装会默认尝试使用 `bash`，而 Windows 上可能没有它。
...
把这个 command 放到你用于 session startup 或 resume 的 Claude Code hook 事件下。`SessionStart` 如果可靠触发，是首选。`UserPromptSubmit` 也可以作为 fallback，但会发送更多注册请求。

不同 Claude Code 版本的 `settings.json` 结构可能不同。关键要求是：

- hook 能从 stdin 收到 Claude Code 的 JSON event
- hook 进程继承 `CLAUDE_CODE_RUN_ID`
- Windows 上 command 指向 `claude_code_session_hook.bat`
- Windows 配置包含 `"shell": "powershell"`

## Linux/macOS Hook 用法

Python hook 跨平台。Linux 或 macOS 上，可以在 Claude Code hook settings 中直接调用 Python 脚本：
...
hook 是 best-effort。失败会记录日志，但 hook 会以 `0` 退出，避免影响 Claude Code。
...
- 把 `<system-reminder>...</system-reminder>` 块按时间顺序拆成 `system` messages
